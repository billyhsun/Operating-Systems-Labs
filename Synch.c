...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          var 
	      prev_interrupt_stat: int
	  prev_interrupt_stat = SetInterruptsTo(DISABLED)
	  while (heldBy != currentThread)
	      ...
	      endIf
         endWhile
	 prev_interrupt_stat = SetInterruptsTo(prev_interrupt_stat)
        endMethod
      ...
      method Unlock ()
	      ...
	      prev_interrupt_state: int
	  prev_interrupt_state = SetInterruptsTo(DISABLED)
	  ...
	  endIf
	  prev_interrupt_state = SetInterruptsTo(prev_interrupt_state)
        endMethod
