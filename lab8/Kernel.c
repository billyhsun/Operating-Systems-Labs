...
-----------------------------  SerialInterruptHandler  --------------------------
  ...
  function SerialInterruptHandler ()
    ...
    --
		currentInterruptStatus = DISABLED
		if serialHasBeenInitialized == true
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
		var
			i, junk, fileDescriptorIndex: int
			strBuffer: array [MAX_STRING_SIZE] of char
			openFile: ptr to OpenFile

		-- Copy filename string from virtual space
		junk = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
		var
			...
			virtAddr, virtPage, offset, copiedSoFar, nextPosInFile, chunkSize, fileSize, destAddr, i: int
			readSuccess: bool
			character: char
		...
		if currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL
			...
			for i = 0 to sizeInBytes - 1
				...
				character = serialDriver.GetChar()
				...
				*(destAddr asPtrTo char) = character
				copiedSoFar = copiedSoFar + 1
		...
		-- If terminal is not used
		-- Check if there is sufficient space (if this part runs to completion)
		virtAddr = buffer asInteger
		virtPage = virtAddr / PAGE_SIZE
		offset = virtAddr % PAGE_SIZE
		...
		fileSize = file.fcb.sizeOfFileInBytes
		...
		-- If no error, now actually read the file
		virtAddr = buffer asInteger
		virtPage = virtAddr / PAGE_SIZE
		offset = virtAddr % PAGE_SIZE
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
		...
		if currentThread.myProcess.fileDescriptor[fileDesc].kind == TERMINAL
			...
			for i = 0 to sizeInBytes - 1
				...
				offset = virtAddr % PAGE_SIZE
				destAddr = currentThread.myProcess.addrSpace.ExtractFrameAddr(virtPage) + offset
				character = *(destAddr asPtrTo char)

				if character == '\n'
					serialDriver.PutChar('\r')
				endIf

				serialDriver.PutChar(character)
				copiedSoFar = copiedSoFar + 1
				buffer = buffer + 1
		...
		-- If no error, now actually write the file
		virtAddr = buffer asInteger
		virtPage = virtAddr / PAGE_SIZE
		offset = virtAddr % PAGE_SIZE
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
		...
		-- if newCurr pos is -1 we set it to N (from lab5)
		if newCurrentPos == -1
			file.currentPos = file.fcb.sizeOfFileInBytes
			fileManager.fileManagerLock.Unlock()
			return file.fcb.sizeOfFileInBytes
		endIf
  ...
  function Handle_Sys_Close (fileDesc: int)
	...
	behavior SerialDriver
		...
		method Init()
			...
			newThread = threadManager.GetANewThread()
			newThread.Init("serialHandlerThread")
			newThread.status = JUST_CREATED
			serialHandlerThread = *newThread
		...
		method GetChar() returns char
			...
			character = getBuffer[getBufferNextOut]
		...
		method SerialHandler()
			...
			while true
				...
				statusRegister = *(serial_status_word_address)

				-- Check input stream
				if (statusRegister & SERIAL_CHARACTER_AVAILABLE_BIT) > 0
					inputCharacter = intToChar(*(serial_data_word_address))
					...
					if getBufferSize == SERIAL_GET_BUFFER_SIZE
						...
						getBuffer[getBufferNextIn] = inputCharacter
				...
				-- Check output stream
				if (statusRegister & SERIAL_OUTPUT_READY_BIT) > 0
					serialLock.Lock()

					if putBufferSize > 0
						inputCharacter = putBuffer[putBufferNextOut]

						putBufferNextOut = (putBufferNextOut + 1) % SERIAL_PUT_BUFFER_SIZE
						putBufferSize = putBufferSize - 1
						*(serial_data_word_address) = charToInt(inputCharacter)
