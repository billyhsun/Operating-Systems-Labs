...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
		var
			...
			junk: int
			strBuffer: array [MAX_STRING_SIZE] of char
			fileDescriptorSlot: int
			file: ptr to OpenFile

		junk = currentThread.myProcess.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)

		-- Cannot handle file if filesize is too large
		if filename arraySize > MAX_STRING_SIZE
			return -1
		endIf

		-- Locate empty slot in fileDescriptor array
		fileDescriptorSlot = -1
		for i = 0 to MAX_FILES_PER_PROCESS - 1
			if currentThread.myProcess.fileDescriptor[i] == null
				fileDescriptorSlot = i
				break
			endIf
		endFor

		-- If no empty slots
		if fileDescriptorSlot == -1
			return fileDescriptorSlot
		endIf
		
  		-- Allocate space to open file
		file = fileManager.Open(&strBuffer)
		if file == null
			return -1
		endIf

		currentThread.myProcess.fileDescriptor[fileDescriptorSlot] = file
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
		var
			file: ptr to OpenFile
			virtAddr, virtPage, offset, copiedSoFar, nextPosInFile, chunkSize, fileSize, destAddr: int
			readSuccess: bool
